...
| 时间轴 | 固定日历顶栏、说明、比例视图空表格 | 后台裁剪当天记录、排序并计算重叠轨道 | 停止时钟；保留有效结果直到数据源变化 |
| 备注 | 固定日历顶栏和列表外壳 | 后台构建备注索引，再后台筛选当天记录 | 保留有效投影；数据源变化后失效 |
| 统计 | 固定日历顶栏、范围菜单、汇总卡片和图表/排行外壳 | 只后台计算当前选中的范围 | 保留当前有效结果；范围或数据变化后失效 |
...
- 仪表盘只根据 `running.isNotEmpty()` 显示“空闲 / 工作中”，不能通过 `firstOrNull()` 把某一条记录冒充为全局状态。
- 仪表盘事件数直接使用进行中记录列表大小。
- 所有具体进行中记录都必须紧接仪表盘展示，不添加重复的“正在进行 / 项数”标题行，也不受首页显示开关控制；以记录 ID 作为 LazyColumn 稳定 key，按开始时间倒序纵向展示。
- 进行中记录使用紧凑的中性色卡片，仅在左侧用记录的分组颜色竖条标识归属，右侧只显示计时，不重复绘制状态圆环或勾。
- 首页只创建一个稳定的共享时钟 `State<Long>`，并且只在首页对应 `contentActive == true` 时启动；`HomeScreen`、`LazyColumn`、仪表盘和卡片外壳禁止读取秒值，只有“当前时间”和每张运行卡片的计时 `Text` 可以读取该状态。这样每秒只重组计时文字，同时禁止每张卡片单独启动时钟协程。
- 结束单项按记录 ID 更新；结束全部必须在同一个 Room 事务中使用同一个结束时间，并在事务完成后统一刷新桌面小组件。
...
- 日历顶栏的周/月 Pager 只负责轻量日期导航。日模式浏览其他周不能修改统计日期或触发计算；周、月模式停稳后可以修改范围基准日期并触发当前所选范围的单次计算。
...
- Android 13 及以上由主内容层提供一个共享 `LayerBackdrop`，供底栏和三个日期页顶部的 MiuiX 胶囊按钮共同采样；禁止每个按钮各自创建 Backdrop。日期顶栏本体保持非玻璃实体背景。
...
- [ ] 设置二级页面没有被批量预创建。
- [ ] 离开时间轴后没有继续每秒扫描记录。
- [ ] 首页共享时钟只在首页停稳后运行，秒值只在计时文字节点读取，没有让首页或列表根节点每秒重组。
- [ ] 统计没有自动计算未选择的范围。
- [ ] 数据变化时缓存正确失效。
