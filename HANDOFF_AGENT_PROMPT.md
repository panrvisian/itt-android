...
- 首页仪表盘与三色旋转开关：首页右上角增加“记录模式”分段切控件（`实时` | `补录`），支持从当天上一事件结束时刻（无记录时默认 00:00）到当前时刻的一键无缝连轴补录；补录模式下若长按补录的事件与当天上一条已完成记录属于同一个事件，则自动将上一条记录的 `endTime` 扩展合并至当前时刻，不产生重复碎段记录；左上角卡片重构为单向顺时针顺滑旋转三色转盘开关（`+120°` 顺时针累加旋转），三角指针垂直底边 100% 紧贴大方块卡片最右侧外边框、尖角向左指向圆心，圆心显示 `红`/`黄`/`绿` 对应状态文字；深色模式下圆环轨道自动切换为深蓝色（`#1E3A5F`），选中色点描边在浅色下为软淡色、深色下为对应深亮暗色。
- 设置滑块与透明度重构：设置滑块（透明度、模糊度、字号）优化为松手渲染模式（`onValueChangeFinished`），拖动过程 120Hz 极速跟手；全局组件透明度全量注入日历顶栏、仪表盘大卡片、分组芯片与底部导航栏，调高透明度时顶栏自动透明，不再遮挡自定义全屏壁纸；高渲染与高耗电选项下方增加橙黄色卡顿与耗电警告提示。
