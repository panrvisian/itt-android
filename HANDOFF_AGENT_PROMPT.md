...
- 首页仪表盘与三色模式联动：首页左上角三色转盘与右上角记录模式（`补录`:红, `克隆`:黄, `实时`:绿）深度联动。左上角圆心显示 `补录`/`克隆`/`实时` 对应汉字，右上角展示当前模式的详细交互说明（补录：顺延并合并上一记录；克隆：长按事件直接按上一已完成记录的时间段复制；实时：即时打卡走秒）。长按事件按当前模式逻辑执行（克隆模式长按事件自动克隆数据库最新一条已完成记录的时间段到该事件）；左上角转盘保留 `+120°` 顺时针累加旋转、右侧 100% 贴边三角指针、深色深蓝轨道与自适应浅/深色描边。
