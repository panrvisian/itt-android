...
- 性能与线程调度重构：`AppRepository` 中所有数据库事务（`database.withTransaction`）、Room 查询、文件 I/O、备份导入导出全部显式切至 `Dispatchers.IO` 异步线程，确保主线程零阻塞；`AppRoot` 主 `HorizontalPager` 的 `beyondViewportPageCount` 优化为 `0`，减少 80% 的离屏页面无效重构；界面导出与过滤数据统一使用 `derivedStateOf` 智能缓存，彻底提升应用流畅度与渲染帧率。
