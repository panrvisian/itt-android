...
- 首页仪表盘与三色模式联动：首页左上角三色转盘与右上角记录模式（`补录`:红, `克隆`:黄, `实时`:绿）深度联动。左上角圆心显示 `补录`/`克隆`/`实时` 对应汉字，右上角展示当前模式的两行分段交互说明。实时打卡模式（及桌面小组件打卡）下结束记录时，若持续时长小于 1 分钟（60 秒），自动忽略不保存入库，应用内并通过顶部精致浮动胶囊气泡反馈 Capsule Toast 提示“记录不足 1 分钟，已忽略 “事件名””；克隆与补录模式下若克隆/补录的时间段与已有的同事件记录相邻或接续，自动合并扩展该记录的 `startTime` 和 `endTime`，防止产生并排碎块；操作完成后均有顶部浮动胶囊气泡反馈并伴随微震；左上角转盘保留 `+120°` 顺时针累加旋转、右侧 100% 贴边三角指针、深色深蓝轨道与自适应浅/深色描边。
- 设置滑块与全屏壁纸视觉重构：设置滑块（透明度、模糊度、字号）优化为松手渲染模式（`onValueChangeFinished`），拖动过程 120Hz 极速跟手；全局组件透明度全量注入日历顶栏、仪表盘大卡片、分组芯片与底部导航栏；全屏顶部叠加柔和暗色渐变遮罩（Top Ambient Scrim Gradient），日历顶栏背景向上延伸覆盖系统状态栏，彻底解决纯白/纯线条自定义壁纸下顶部文字撞色模糊与状态栏硬割裂断层问题；三色转盘卡片背景设置保底不透明度与对比度轮廓，保证在任意壁纸下均清晰可读。
