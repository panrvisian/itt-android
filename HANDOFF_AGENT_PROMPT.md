...
- 时间轴与日历交互升级：展开日历中选择日期后保持开启不自动折叠；时间轴色块重叠判定统一按分钟级对齐（同一分钟内结束与开始自动上下无缝排列，不并排分列）；今天的时间轴当前时间线实时平滑往下移动与更新；统一时间轴灰色整点线与红色当前时间线的 Y 轴坐标基准，彻底消除缩小视图下红线移至 8 点灰色线上方的倒错问题；时间轴顶栏左上角增加“单日日志导出”图标按钮，点击后触发二级格式选择弹窗，支持将选定日期的记录导出为包含当天 CSV 数据表与备注照片的 ZIP 压缩包，或仅导出纯 CSV 表格，导出完成后顶部平滑弹出自适应胶囊气泡通知。
