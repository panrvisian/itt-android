...
- 首页仪表盘与三色旋转开关：首页右上角增加“记录模式”分段切控件（`实时` | `补录`），支持从当天上一事件结束时刻（无记录时默认 00:00）到当前时刻的一键无缝连轴补录；左上角卡片重构为带右侧固定尖锐三角指针（`▶`）与圆心文字（`红`/`黄`/`绿`）的平滑旋转三色转盘开关，每次点击平滑旋转将选中色点转动至 3 点钟最右侧，卡片背景随状态淡化渐变，深色模式自动应用对应的精致深色描边。
