...
- 首页仪表盘与三色旋转开关：首页右上角增加“记录模式”分段切控件（`实时` | `补录`），支持从当天上一事件结束时刻（无记录时默认 00:00）到当前时刻的一键无缝连轴补录；左上角卡片重构为单向顺时针顺滑旋转三色转盘开关（`+120°` 顺时针累加旋转），三角指针垂直底边 100% 紧贴大方块卡片最右侧外边框、尖角向左指向圆心，圆心显示 `红`/`黄`/`绿` 对应状态文字；深色模式下圆环轨道自动切换为深蓝色（`#1E3A5F`），选中色点描边在浅色下为软淡色、深色下为对应深亮暗色。
