...
- 首页仪表盘与三色模式联动：首页左上角三色转盘与右上角记录模式（`补录`:红, `克隆`:黄, `实时`:绿）深度联动。左上角圆心显示 `补录`/`克隆`/`实时` 对应汉字，右上角展示当前模式的两行分段交互说明（补录：填满上一结束时间至此刻 \n (同事件长按自动合并)；克隆：复制上一已完成记录时间段 \n (同事件长按自动合并)；实时：即时开启走秒与计时 \n (再次长按即刻结束)）。长按事件按当前模式逻辑执行（克隆与补录模式下若克隆/补录的时间段与已有的同事件记录相邻或接续，自动合并扩展该记录的 `startTime` 和 `endTime`，防止产生并排碎块）；补录与克隆成功后从屏幕顶部滑出 2.2 秒精致浮动胶囊气泡反馈 Capsule Toast 并伴随微震；左上角转盘保留 `+120°` 顺时针累加旋转、右侧 100% 贴边三角指针、深色深蓝轨道与自适应浅/深色描边。
