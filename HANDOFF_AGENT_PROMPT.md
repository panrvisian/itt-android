...
- 事件块长按 0.5 秒开始记录；下方进行中记录卡片长按结束单条记录，短按打开详情，并按设置决定是否震动。
- 首页左侧状态卡为正方形，宽度与右侧卡片列完全相同；第一行固定为“当前”，第二行在“空闲 / 工作中”之间切换，垂直居中且左对齐，不显示具体事件或第三行说明。工作状态使用加粗且部分裁切的大圆环与勾作为背景；长按“工作中”状态卡会在一个 Room 事务中以完全相同的结束时间结束全部进行中记录。
- 首页右侧上方显示当前时间，右侧下方固定显示“事件数”；空闲为 0，工作时显示当前进行中的记录数量。
- 具体进行中事件无论一条还是多条，都必须紧接仪表盘按开始时间倒序纵向堆叠，不显示重复的“正在进行 / 项数”标题，也不提供隐藏开关。记录使用紧凑的中性色卡片，左侧以记录颜色竖条配合分组、事件名称，右侧仅以 `M:SS` 或 `H:MM:SS` 显示独立运行时长，不绘制圆环或勾。所有卡片共享首页唯一的稳定时钟状态，该时钟只在首页停稳后运行；只有时间文字读取秒值，禁止让 `HomeScreen`、`LazyColumn` 或卡片外壳每秒重组，也不得为每条记录创建定时协程。
- “未分组”是系统分组，固定在分组列表底部且不可排序；其下方固定保留“新建分组”入口。
- 首页横向分组选择栏（`HorizontalGroupSelectorBar`）挂载手势边界拦截（`stopParentHorizontalScrollAtBounds`），当向左或向右滑动触顶/触底时，手势偏移量在本地消化，不传导至外层 Pager，防止滑动分组标签栏拖动整个主界面。
...
- 时间轴、备注和统计共用固定的非玻璃 MiuiX 日历顶栏，不重复显示底栏已有的页面名称；日期控件和日历始终位于正文滚动区之外。第一行左侧为“今天/本周/本月”，中间的日模式日期只显示“月 日”，右侧为页面操作。第一行所有按钮统一使用 MiuiX 液态玻璃胶囊样式。
- 日视图折叠为周日开始的七日周条，点击日期才更新正文；左右滑动周条只浏览相邻周，不修改已选日期。点击居中日期或周条下方的展开带会让同一个顶栏原地增高为固定六行的完整月历，不使用弹窗或遮罩；月历横滑切月。在月历中点击日期或点击“今天”后更新正文并保持日历展开状态，不自动折叠，方便连续切换日期；仅在点击顶部收起箭头、点击标题或滚动正文时自动收起并恢复对应周条。
...
- 重叠计算统一按分钟级对齐（`timelineMinute`），当一条记录的结束时间与下一条记录的开始时间在同一分钟内时，判定为顺畅接续而非重叠，在时间轴上全宽上下排列，不进行左右并排分列；只有存在真实时间重叠时才沿用横向分列和宽度规则。
- 紧凑视图不显示小时刻度、横线、当前时间线，也不启用缩放。
- 当前时间线只在比例视图且所选日期为今天时显示，且只要处于今天视图或存在进行中记录，时钟均保持激活并每秒驱动更新，确保当前时间线平滑往下实时移动。
- 启动预加载和页面滑动期间只组合时间轴控件与空表格；页面完全停稳后才在 `Dispatchers.Default` 构建当日记录布局。
...
- 备注页使用独立日期状态和统一日历顶栏；与时间轴、统计互不共享日期。
...
- 统计范围由顶栏右上角的 MiuiX 下拉菜单选择“天、周、月、学期”，默认“天”。天模式的周条只浏览、点击日期后刷新；周模式横滑整周后立即刷新且不突出单日；月模式折叠时只显示月份，横滑立即切月，点击月份可原地展开月历；学期模式暂时只有设置中已有的一个学期，使用 MiuiX 中央吸附滚轮展示。
- 三个日期页面各自维护独立日期状态。日历周从周日开始；统计周范围也固定按周日到周六计算。
...
- 全局 UI/UX 重构与高性能导航（v2.25 / versionCode 30）：
  ...
  - **内容驱动的分阶段启动**：不再使用固定 900ms 等待或 Compose 二次开屏层。系统 Splash 使用 `drawable-nodpi/splash_icon.png` 产品标志并等待首页真实数据首发；首页完成一帧后，再按帧依次预组合时间轴、备注、统计和设置页面外壳，全部完成后自动退出。页面外壳随后保持驻留，记录投影与统计计算不参与开屏。
...
- 备注文字在同一跨天事件的各切分段间共享并双向同步；在任意切分段（首段或续段）编辑/修改/删除备注文字，全组切分段记录同步更新；备注图片和图片文件统一归属并存储于第一段。
...
- 时间轴与日历交互升级：展开日历中选择日期后保持开启不自动折叠；时间轴色块重叠判定统一按分钟级对齐（同一分钟内结束与开始自动上下无缝排列，不并排分列）；今天的时间轴当前时间线实时平滑往下移动与更新；统一时间轴灰色整点线与红色当前时间线的 Y 轴坐标基准，彻底消除缩小视图下红线移至 8 点灰色线上方的倒错问题。
- 设置“导入/导出”新增按时间段导出卡片：彻底移除时间轴顶栏临时导出按钮以保持极简；导出功能整合进入“设置 ➔ 导入/导出”，提供全量系统备份卡片与按时间段导出卡片。按时间段导出支持分段切换 `[ 单日 | 单周 | 单月 | 自定义 ]`，无缝嵌入 `MiuixCalendarHeader` 展开式日历/周历/月历选择器与自定义区间选择，可分别导出含对应范围内全备注照片的 ZIP 压缩包或纯 CSV 数据表，导出成功后由顶部自适应胶囊气泡给出提示。
- 首页仪表盘与记录模式：左上角卡片为仪表盘状态卡片（显示“当前 空闲/工作中”与运行环形动画，长按“工作中”结束全部）；右上角为“记录模式”分段切控件（`[ 实时 | 补录 ]`）；补录模式下支持从当天上一事件结束时刻（无记录默认 00:00）到当前时刻的一键无缝连轴补录，若同事件相邻/重叠自动延长合并；实时打卡模式（及桌面小组件）下结束记录时，若持续时长小于 1 分钟（60秒），自动忽略不保存入库，并通过顶部精致浮动胶囊气泡反馈 Capsule Toast 提示“记录不足 1 分钟，已忽略 “事件名””；补录成功或合并后均有顶部浮动胶囊气泡反馈并伴随微震。
- 设置滑块与全屏壁纸视觉重构：设置滑块（透明度、模糊度、字号）优化为松手渲染模式（`onValueChangeFinished`），拖动过程 120Hz 极速跟手；移除设置页静态黄色警告文字，改用复用的顶部浮动胶囊气泡（Capsule Toast）在切换/调高高阶特效时自适应弹出耗电与渲染告知；胶囊气泡外观全量响应当前明暗主题模式（浅色下白底深字，深色下黑底白字）；全局组件透明度全量注入日历顶栏、仪表盘大卡片、分组芯片与底部导航栏；全屏顶部叠加柔和暗色渐变遮罩（Top Ambient Scrim Gradient），日历顶栏背景向上延伸覆盖系统状态栏，彻底解决纯白/纯线条自定义壁纸下顶部文字撞色模糊与状态栏硬割裂断层问题。
